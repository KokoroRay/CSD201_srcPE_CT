...
//            System.out.print(node.getData() + "-> ");
    ...
    public int getOrder(int data) {
        return getOrder(root, data, 0);
    }

    public int getOrder(BSTNode node, int data, int order) {
        if (node == null) {
            return 0;
        }
        ...
        int leftOrder = getOrder(node.getLeftChild(), data, 2*order + 1);
        if (leftOrder != 0) {
            return leftOrder;
        }

        int rightOrder = getOrder(node.getRightChild(), data, 2*order + 2);
        return rightOrder;
    }
