...
//            System.out.print(node.getData() + "-> ");
    ...
    public void Sibling(int data) {
        BSTNode node = findNode(data);
        if (node.hasParent(node)) {
            BSTNode node2 = node.getParent();
            if (node2.getLeftChild() == null || node2.getRightChild() == null) {
                resultSibling += null + ", ";
            } else {
                if (node2.getLeftChild().getData() == data) {
                    resultSibling += node2.getRightChild().getData() + ", ";
                } else {
                    resultSibling += node2.getLeftChild().getData() + ", ";
                }
            }
        }
    }

    public String getResultSibling() {
        return resultSibling;
    }
