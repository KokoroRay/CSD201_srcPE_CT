...
//        }
//    }
    public void CalLevelTree() {
        int tp = 0;
        CalLevelTree(root, tp);
    }

    public void CalLevelTree(BSTNode node, int level) {
        if (node == null) {
            return;
        }
        node.setLevel(level);
        CalLevelTree(node.getLeftChild(), level + 1);
        CalLevelTree(node.getRightChild(), level + 1);
    }

    public int displayLevel(BSTNode node) {
...
//            System.out.print(node.getData() + "-> ");
    ...
    public void findParent(BSTNode node) {
        if (root == null) {
            return;
        }
        
        if (node.isRoot()){
            result += node.getData() + "[" + node.getParent() + "],";
        }else{
            result += node.getData() + "[" + node.getParent().getData() + "],";
        }
        ...
        if (node.hasLeftChild()) {
            findParent(node.getLeftChild());
        }

        if (node.hasRightChild()) {
            findParent(node.getRightChild());
        }
    }
}
