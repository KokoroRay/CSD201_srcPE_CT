...
public class Graph {
    ...
    public void writeData(String fileName){
        findIsolated(); //listIsolated
        try {
            FileWriter out = new FileWriter(fileName);
            if(listIsolated.size() == 0){
                out.write("There is a connected graph");
            }else{
                out.write(listIsolated.size() + "\r\n");
                String resultIsolated = "";
                for (Vertex v : listIsolated) {
                    resultIsolated += "," + v.getData();
                }
