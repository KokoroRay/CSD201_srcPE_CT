...
public class Graph {
    ...
    private List<int[]> bridges; // Danh sách lưu các cạnh cắt
    ...
    public void updateEdge(int startVertex, int endVertex, int newValue) {
        graph[startVertex][endVertex] = newValue;
        graph[endVertex][startVertex] = newValue;
    }
...
//        result = result.trim();
     ...
     * ---------------------- Cut Edge --------------------------------
     */
    public void resetCutEdge() {
        Arrays.fill(parent, -1);
        resetResult();
    }

    // Hàm tìm các cạnh cắt (cut edge)
    public void findCutEdges() {
    ...
    // Hàm DFS tìm các cạnh cắt
    private void dfsCutEdge(int u) {
        isVisited[u] = true;
        discoveryTime[u] = low[u] = ++time; // Cập nhật thời gian phát hiện
        for (int v = 0; v < NumberOfVertices; v++) {
            if (graph[u][v] == 1) { // Chỉ duyệt các đỉnh kề với u
                if (!isVisited[v]) {
                    parent[v] = u;
                    dfsCutEdge(v);
                    // Sau khi DFS, cập nhật low của u
                    low[u] = Math.min(low[u], low[v]);

                    // Kiểm tra nếu (u, v) là cạnh cắt
                    if (low[v] > discoveryTime[u]) {
                        bridges.add(new int[]{u, v}); // (u, v) là cạnh cắt
                    }
                } else if (v != parent[u]) {
                    // Nếu v đã được thăm và không phải cha của u, cập nhật low của u
                    low[u] = Math.min(low[u], discoveryTime[v]);
                }
            }
        }
